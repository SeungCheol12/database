...
-- 제약조건(데이터 무결성 유지) CONSTRAINT 
-- 1. 빈 값을 허용하지 않는 NOT NULL
-- 2. 중복 값을 허용하지 않는 UNIQUE
-- 3. 유일하게 하나만 존재하는 PRIMARY KEY
-- 4. 다른 테이블과 관계를 맺는 FOREIGN KEY
-- 5. 설정한 조건식을 만족하는 데이터CHECK
-- 6. 기본값을 지정하는 DEFAULT

-- 데이터 뮤결성 : 데이터의 정확성과 일관성 보장
...
INSERT INTO TABLE_NOTNULL VALUES ('test01', NULL , '010-1234-5678'); -- 오류 발생
...
-- 제약 조건 이름 변경
ALTER TABLE TABLE_NOTNULL RENAME CONSTRAINT SYS_C008359 TO TBLNN_TEL_NN;
...
INSERT INTO TABLE_UNIQUE VALUES ('test01', 'test02', '010-1234-7889');
-- UNIQUE 는 NULL 값에 중복의 의미를 부여하지 않음
INSERT INTO TABLE_UNIQUE VALUES (NULL, 'test02', '010-1234-7889');

INSERT INTO TABLE_UNIQUE VALUES ('test02', 'test02', '010-1234-7889');
-- 무결성 제약 조건(SCOTT.SYS_C008361)에 위배됩니다
UPDATE TABLE_UNIQUE tu SET LOGIN_ID = 'test01';
...
--  UNIQUE 제약 조건 이름 지정
CREATE TABLE TABLE_UNIQUE(
	LOGIN_ID VARCHAR2(20) CONSTRAINT CON_UNI UNIQUE ,
	LOGIN_PWD VARCHAR2(20) CONSTRAINT CON_NON NOT NULL,
	TEL VARCHAR2(20)
);
...
ALTER TABLE TABLE_UNIQUE ADD CONSTRAINT uk_tel UNIQUE(TEL);
...
-- 4. FOREIGN KEY(외래키)
-- 다른 테이블과 관계를 맺을 때
CREATE TABLE DEPT_FK(DEPTNO NUMBER(2)PRIMARY KEY, DNAME VARCHAR2(14),
LOC VARCHAR2(13));

CREATE TABLE EMP_FK(EMPNO NUMBER(4), 
ENAME VARCHAR2(10), JOB VARCHAR2(9), MGR NUMBER(4), HIREDATE DATE, 
SAL NUMBER(7, 2), COMM NUMBER(7, 2), DEPTNO NUMBER(2) REFERENCES DEPT_FK(DEPTNO)
);

-- 무결성 제약조건(SCOTT.SYS_C008368)이 위배되었습니다- 부모 키가 없습니다
INSERT INTO EMP_FK  VALUES (7480, 'A', 'SALESMAN', 7698, SYSDATE, 800, 0, 50);

-- 입력 시 부모 테이블 데이터 먼저 입력 후 자식 테이블 데이터 입력

INSERT INTO DEPT_FK VALUES (10, 'DATABASE', 'SEOUL');
INSERT INTO EMP_FK  VALUES (7480, 'A', 'SALESMAN', 7698, SYSDATE, 800, 0, 10);

-- 무결성 제약조건(SCOTT.SYS_C008368)이 위배되었습니다- 부모 키가 없습니다
UPDATE EMP_FK SET DEPTNO = 20 WHERE EMPNO = 7480;


-- 삭제 시
-- 자식 레코드 존재시 부모 삭제 불가
-- 1) 자식 레코드 먼저 삭제 후 부모 레코드 삭제
-- 무결성 제약조건(SCOTT.SYS_C008368)이 위배되었습니다- 자식 레코드가 발견되었습니다
DELETE FROM DEPT_FK WHERE DEPTNO = 10;

DELETE FROM EMP_FK WHERE EMPNO = 7480;
DELETE FROM DEPT_FK WHERE DEPTNO = 10;

DROP TABLE DEPT_FK ;
DROP TABLE EMP_FK ;

-- 제약 조건명 + 부모 데이터 사제 시 자식 데이터 처리 방법 지정
-- 1) ON DELETE CASCADE : 부모 데이터 삭제 시 참조하는 데이터도 함께 삭제
-- 2) ON DELETE SET NULL : 부모 데이터 삭제 시 참조하는 데이터에 NULL 설정
CREATE TABLE DEPT_FK(DEPTNO NUMBER(2)CONSTRAINT DEPT_DEPTNO_PK PRIMARY KEY, DNAME VARCHAR2(14),
LOC VARCHAR2(13));

CREATE TABLE EMP_FK(EMPNO NUMBER(4), 
ENAME VARCHAR2(10)CONSTRAINT EMPFK_EMPNO_PK PRIMARY KEY , JOB VARCHAR2(9), MGR NUMBER(4), HIREDATE DATE, 
SAL NUMBER(7, 2), COMM NUMBER(7, 2), 
DEPTNO NUMBER(2)CONSTRAINT EMPFK_EMPNO_FK REFERENCES DEPT_FK(DEPTNO) ON DELETE SET NULL
);
...
CREATE TABLE TABLE_CHECK(
	LOGIN_ID VARCHAR2(20) NOT NULL ,
	LOGIN_PWD VARCHAR2(20) CONSTRAINT CON_NON1 CHECK (LENGTH(LOGIN_PWD) > 3),
	TEL VARCHAR2(20)
);
-- 체크 제약조건(SCOTT.CON_NON1)이 위배되었습니다
...
-- 체크 제약조건(SCOTT.TBLCK_AGE_CK)이 위배되었습니다
INSERT INTO TABLE_CHECK VALUES ('test01', 'tes', 9,'010-1234-5678');
...
INSERT INTO TABLE_DEFAULT VALUES ('test02',NULL ,'010-1234-7891');
